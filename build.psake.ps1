...
Task Build -depends Init, Clean, BeforeBuild, StageFiles {

    $Files = Get-ChildItem -Path $SrcRootDir -Filter *.ps1 -File -Recurse

    $Files |
        Get-Content |
        Add-Content -Path "$ModuleOutDir\$ModuleName.psm1"

    $PublicFunctionNames = $Files | Where-Object {$_.Directory.Name -eq 'Public'} |
    ...
    Update-Metadata -Path "$ModuleOutDir\$ModuleName.psd1" -PropertyName FunctionsToExport -Value $PublicFunctionNames
}

Task Analyze -depends StageFiles, Build, AfterBuild `
...
Task Sign -depends StageFiles -requiredVariables CertPath, SettingsPath, ScriptSigningEnabled, Build, AfterBuild {
...
Task Test -depends Build, AfterBuild, Analyze -requiredVariables TestRootDir, ModuleName, CodeCoverageEnabled, CodeCoverageFiles {
